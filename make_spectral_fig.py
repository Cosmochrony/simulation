...
def extract_series(results: List[RunResult], mode_indices: List[int]) -> Dict[int, Tuple[List[int], List[float], List[float]]]:
    """
...
def plot_sep_scan(results_sym: List[RunResult], outfile: str = "spectral_scan_sep.pdf", mode_indices=(0, 1, 2, 3)):
    ...
    for n, (seps, lams, iprs) in series.items():
        ax1.plot(seps, lams, marker="o", linestyle="-", label=f"$\\lambda_{n}$")
    ...
    # Panel (b): IPR
    for n, (seps, lams, iprs) in series.items():
        ax2.plot(seps, iprs, marker="o", linestyle="-", label=f"IPR($n={n}$)")
