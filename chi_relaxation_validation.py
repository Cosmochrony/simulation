...
def S_local(u: np.ndarray, K0: float, chi_c: float, c: float, no_softening: bool) -> np.ndarray:
  """
...
def rhs_micro(u: np.ndarray, c: float, kappa: float, K0: float, chi_c: float, no_softening: bool) -> np.ndarray:
  """RHS of micro evolution (without dt): R(u) + kappa Laplacian(u)."""
  S = S_local(u, K0=K0, chi_c=chi_c, c=c, no_softening=no_softening)
  R = c * np.sqrt(np.clip(1.0 - S, 0.0, None))  # saturation if S>1
  return R + kappa * laplacian(u)


def evolve(u: np.ndarray, dt: float, c: float, kappa: float, K0: float, chi_c: float, no_softening: bool) -> np.ndarray:
  """Explicit Euler step."""
  return u + dt * rhs_micro(u, c=c, kappa=kappa, K0=K0, chi_c=chi_c, no_softening=no_softening)
...
def run_case(cfg: CaseConfig,
  ...
  for t in range(steps):
    # Evolve micro
    chi = evolve(chi, dt=dt, c=c, kappa=cfg.kappa, K0=K0, chi_c=cfg.chi_c, no_softening=no_softening)

    # Compute RHS micro
    rhs_curr_micro = rhs_micro(
      chi, c=c, kappa=cfg.kappa, K0=K0, chi_c=cfg.chi_c, no_softening=no_softening
    )
    ...
    S = S_local(chi, K0=K0, chi_c=cfg.chi_c, c=c, no_softening=no_softening)
...
def main() -> None:
  ...
  ap.add_argument(
    "--no_softening",
    action="store_true",
    help="Disable K softening: use K_ij = K0 instead of K0/(1+(Δchi/chi_c)^2).",
  )
